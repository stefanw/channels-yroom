...
async def test_yroom_consumer():
    ...
    SYNC_STEP_1 = b"\x00\x00\x01\x00\x01\x01\x00"
    ...
    worker_communicator = ApplicationCommunicator(
        worker, {"type": "channel", "channel": settings.YROOM_CHANNEL_NAME}
    )
    ...
    message = await forward_to_yroom_worker(channel_layer, worker_communicator)
...
async def test_yroom_consumer_with_snapshot(settings):
    settings.YROOM_REMOVE_ROOM_DELAY = 0
    settings.YROOM_STORAGE_BACKEND = "channels_yroom.storage.YDocDatabaseStorage"
    ...
    SYNC_STEP_1 = b"\x00\x00\x07\x01\xe9\xdb\x9a\x90\x01\x06\x01\x01\x00"
    SYNC_STEP_2 = b"\x00\x01\x02\x00\x00"
